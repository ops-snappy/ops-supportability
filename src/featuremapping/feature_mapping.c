...
parse_feature_mapping_yaml(void)
{
  ...
  while(event.type != YAML_STREAM_END_EVENT){
    if(event.type == YAML_SCALAR_EVENT) {
      event_value =
      feature_mapping_check_key ((const char*) event.data.scalar.value);
      switch(event_value) {
        case VALUE:
        {
          switch(current_state) {
            ...
            {
              curr_daemon  = NULL;
              curr_feature = feature_mapping_add_feature(curr_feature,
           ...
           case FEATURE_DESC:
            {
              feature_mapping_add_feature_desc(curr_feature,
                (const char*)event.data.scalar.value);
                break;
            }
            case DAEMON:
            {
              curr_daemon = feature_mapping_add_daemon(curr_daemon,
                (const char*) event.data.scalar.value);
